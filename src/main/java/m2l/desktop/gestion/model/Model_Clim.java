...
public class Model_Clim {
    ...
    public static void insertClimatiseur(Climatiseur c)
    {
        try
        {
            ...
            int surfMin = c.getSurfaceMin();
            int surfMax = c.getSurfaceMax();
            int conso = c.getConso();
            int son = c.getNiveau_sonore();
            String classe = c.getClasse();


            //définition de la requête
            String sql = "insert into climatiseurs values (null,\""+marque+"\",\""+modele+"\", "+puissance+","+surfMin+","+surfMax+",1,\""+classe+"\","+conso+","+son+")";
            System.out.println("requête :"+sql);
            //exécution de la requête
